import { Truck, ShieldCheck, MapPin, Award, CheckCircle2 } from 'lucide-react';
 ...
 */
const BrandAbout = ({ brand, language }) => {
  ...
  return (
    <section id="about-section" className="py-20 lg:py-28 px-6 sm:px-12 xl:px-20 bg-white w-full border-t border-slate-200/40 relative overflow-hidden">
      ...
      <div className="max-w-7xl xl:max-w-[1360px] 2xl:max-w-[1560px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
        ...
        {/* Left Side (7/12): Strategic Relationship & Values */}
        <div className="lg:col-span-7 flex flex-col text-left items-start animate-fade-in-up">
          ...
          <h2 className="text-3xl sm:text-4xl xl:text-5xl font-extrabold text-primary-dark font-display tracking-tight leading-[1.15] mb-6">
            {data.title}
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-2">
            <a
              href="#"
              className={`inline-flex items-center gap-2 px-6 py-3 rounded-full text-xs font-extrabold transition-all duration-300 shadow-lg text-white ${brand.accentBg} ${brand.accentHover} transform hover:scale-105 active:scale-95`}
            >
              <span>{isEs ? 'Ver Registro Importador' : 'View Importer Register'}</span>
            </a>
            
            <a
              href="#"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-xs font-extrabold transition-all duration-300 border border-slate-200 bg-white hover:bg-slate-50 text-primary-dark shadow-md transform hover:scale-105 active:scale-95"
            >
              <span>{isEs ? 'Certificación Oficial' : 'Official Certification'}</span>
            </a>
          </div>

        </div>

        {/* Right Side (5/12): Premium Institutional Image & Floating Info Cards */}
        <div className="lg:col-span-5 w-full flex items-center justify-center relative mt-10 lg:mt-0">
          <div className="relative w-full max-w-[420px] aspect-[1.25] md:aspect-[1.3] group">
            ...
            {/* Central Badge Overlay: Suministro Directo (Bullet 0) */}
            {data.bullets[0] && (
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-2 rounded-2xl bg-slate-950/85 backdrop-blur-md border border-white/15 shadow-xl text-center z-10 pointer-events-none">
                <span className="text-[10px] sm:text-xs font-extrabold text-white uppercase tracking-widest font-mono">
                  {data.bullets[0].title}
                </span>
              </div>
            )}

            {/* Floating Card 1: Autenticidad Garantizada (Bullet 1) */}
            {data.bullets[1] && (
            ...
            {/* Floating Card 2: Cobertura Nacional (Bullet 2) */}
            {data.bullets[2] && (
