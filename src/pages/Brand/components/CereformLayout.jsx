...
export const CereformLayout = ({ brand, language: propLanguage, onBackToHome }) => {
  ...
  return (
    <div className="min-h-screen bg-white w-full flex flex-col justify-between overflow-x-hidden text-slate-800 font-sans selection:bg-[#3e97b6]/30">
      ...
      <header className={`fixed top-0 left-0 w-full z-50 px-3 xs:px-4 sm:px-8 lg:px-20 pt-3 sm:pt-6 transition-all duration-500 transform ${isNavbarVisible ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'
        }`}>
        <div
          className={`max-w-7xl xl:max-w-[1360px] 2xl:max-w-[1560px] mx-auto rounded-full transition-all duration-500 ${isScrolled
              ...
              {Object.keys(sections).map((key) => {
                ...
                return (
                  <button
                    ...
                    className={`text-xs 2xl:text-sm font-semibold transition-all duration-200 relative py-1 focus:outline-none cursor-pointer after:content-[''] after:absolute after:bottom-0 after:left-0 after:h-[2px] after:transition-all after:duration-300 ${isActive
      ...
      >
        ...
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none select-none">
          {bgImages.map((src, index) => (
            <div
              key={src}
              className={`absolute inset-0 bg-cover bg-center transition-opacity duration-[2000ms] ease-in-out ${index === bgIndex ? 'opacity-40 md:opacity-50' : 'opacity-0'
                }`}
